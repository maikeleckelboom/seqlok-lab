import { type CpJsonObject, type CpPhase, isCpJsonObject } from "../protocol";

import type { CoprocessorRuntimeErrorCode } from "../errors";

/**
 * Internal runtime phase used by the coprocessor runtime to distinguish between:
 * - `"setup"`: non-real-time lifecycle (loading/initialization)
 * - `"rt"`: real-time audio processing
 *
 * This type is intentionally narrower than the wire protocol phase (`CpPhase`),
...
/**
 * Extracts a coprocessor-runtime error code from an unknown thrown value.
 *
 ...
 * @returns The extracted {@link CoprocessorRuntimeErrorCode} if present, otherwise `undefined`.
 */
export function getErrorCode(
  err: unknown,
): CoprocessorRuntimeErrorCode | undefined {
  if (!isRecord(err) || !("code" in err)) {
    return undefined;
  }
  const code = (err as { readonly code?: unknown }).code;
  return typeof code === "string"
    ? (code as CoprocessorRuntimeErrorCode)
    : undefined;
}
 ...
 *
 * Only values accepted by {@link isCpJsonObject} are returned.
 *
 * @param err - Unknown thrown value.
 * @returns A {@link CpJsonObject} if a valid `details` payload is present, otherwise `undefined`.
 */
export function getErrorDetails(err: unknown): CpJsonObject | undefined {
  if (!isRecord(err) || !("details" in err)) {
    return undefined;
  }
  const details = (err as { readonly details?: unknown }).details;
  return isCpJsonObject(details) ? details : undefined;
}
 ...
 */
export function phaseForWire(phase: Phase): CpPhase {

...
/**
 * High-level lifecycle state of the coprocessor instance hosted by this processor.
 *
 ...
 */
export type CoprocessorState = "idle" | "loading" | "ready" | "faulted";

/**
 * Internal phase used for enforcing RT constraints (notably: no allocation during `"rt"`).
 *
 * This is converted to the protocol-level {@link CpPhase} with {@link phaseForWire}.
 */
export type CoprocessorPhase = "setup" | "rt";
...
/**
 * Base class for AudioWorklet processors which host a WASM-backed coprocessor module
 * (Emscripten MODULARIZE/ES6 factory style).
 *
 * Responsibilities:
 * - Receives and validates `cp:mount` messages.
 ...
 *
 * @typeParam T - Concrete Emscripten module type exported by the coprocessor build.
 */
export abstract class CoprocessorAudioWorkletProcessor<
...
> extends AudioWorkletProcessor {
  ...
  constructor() {
    ...
    // Route cp:mount internally, and forward everything else to subclass hook.
  ...
  /**
   * Returns the registry of bundled coprocessor factories (key → Emscripten factory).
   *
   * Used when `cp:mount` does not provide a dynamic wrapper (`wrapperJs`).
   *
   ...
   * @throws Coprocessor runtime errors when the module is unavailable/not ready or allocation is forbidden.
   */
  protected malloc(bytes: number): number {
    ...
    if (!mod) {
      throw createCoprocessorRuntimeError("moduleNotReady", {
        op: "malloc",
        state: this.#state,
      });
    }
   ...
   * @throws Coprocessor runtime errors when the module is unavailable/not ready or freeing is forbidden.
   */
  ...
  /**
   * Handles a message that claims to be `cp:mount` but fails the structural guards.
   *
  ...
  #handleInvalidMountMessage(data: Record<string, unknown>): void {
    ...
    if (key.length === 0) {
      err = createCoprocessorRuntimeError("emptyKey", { op: "cp:mount" });
    } else if (!isSeq(data.seq)) {
      err = createCoprocessorRuntimeError("invalidMountMessage", {
      ...
      try {
        assertWasmBinary(data.wasmBytes, "cp:mount");
        err = createCoprocessorRuntimeError("invalidMountMessage", {
   ...
   * - On success: posts `cp:ready`.
   * - On failure: posts `cp:error` with structured details.
   *
   * Concurrency note:
   ...
   */
  async #handleMount(msg: CpMountMessage): Promise<void> {
    if (this.#state === "loading") {
      const err = createCoprocessorRuntimeError("busyLoading", {
   ...
   * @throws Coprocessor runtime error when the bundled factory key is not registered.
   */
  #selectFactory(msg: CpMountMessage): EmscriptenFactory<T> {
   ...
   * - `print/printErr` forward logs to the host via `cp:log`, while optionally
  ...
  /**
   * Posts a `cp:log` message to the host.
   *
   * @param key - Coprocessor key (mount key).
   * @param seq - Mount sequence number.
   * @param level - Log severity level.
   * @param msg - Log message text.
   */
  #postLog(key: string, seq: number, level: CpLogLevel, msg: string): void {
    this.#post({ type: "cp:log", key, seq, level, msg });
  }

  /**
   * Posts a `cp:error` message to the host, including optional structured fields.
   *
   ...
   */
  #post(message: CpMessageOut): void {

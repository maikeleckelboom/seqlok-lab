...
/**
 * @fileoverview
 * Controller-side parameter write micro-benchmarks.
 *
 * Scenarios:
 * - single-scalar `params.set(key, value)`
 * - small multi-key patches via `params.update({ ... })`
 * - array writes via `params.stage(key, cb)`
 *
 * These benches document relative hot-path costs on the controller side,
 * not micro-optimizations of individual branches.
 */
